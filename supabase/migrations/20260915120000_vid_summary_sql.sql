-- 「导出唯一VID汇总」改在数据库里聚合。
--
-- 原来的做法是把整月归并行（2026-08 有 25 万组）分页拉进 Edge Function 再用 JS 聚合：
-- 250 多次 HTTP 往返 + 25 万行 JS 聚合，跟快照重算撞的是同一堵墙
-- （客户端报 `Failed to send a request to the Edge Function`，其实是请求被超时/CPU 掐断）。
--
-- 这里一条 SQL 出结果：按 (站点, VID, 商品ID) 聚合、关联 SKU、带上该月最新快照的归属，
-- 再打成 JSON 分页返回（单行，绕开 PostgREST 的 1000 行上限）。
...
-- 加个索引让 (站点, VID) 的归属查找走索引，否则 25 万次 LATERAL 会退化成全表扫
