...
export function UploadView({
  ...
  const refinalize = async (u: UploadRec): Promise<void> => {
  ...
  /**
   * 可清除的批次 = 所有没入库成功的（上传中 / 失败）。
   * 原来还要求「创建超过 15 分钟」才算卡住，结果刚失败的批次按钮是灰的，反而干预不了；
   * 归并失败的批次留着也没用（报表只读 READY 批次），所以只要不是 READY 就允许清除。
   */
  const staleUploads = React.useMemo(() => history.filter((u) => u.status !== "READY"), [history]);

  const clearStale = async () => {
    if (!staleUploads.length) return;
    const list = staleUploads.slice(0, 10).map((u) => `· ${u.country} ${u.month} ${u.file_name}`).join("\n");
    if (!window.confirm(
      `将清除 ${staleUploads.length} 条未完成（上传中/失败）的上传记录，连同已入库的原始行一并删除，清除后可重新上传：\n\n${list}${staleUploads.length > 10 ? "\n…" : ""}`,
    )) return;
    setClearing(true);
    let ok = 0;
    let fail = 0;
        ...
        <CardHeader className="pb-3">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Button
              ...
              title="删除所有未入库成功的批次（上传中/失败），删完可以重新上传"
            >
              <Eraser className={`h-4 w-4 mr-1.5 ${clearing ? "animate-pulse" : ""}`} />
              清除未完成记录{staleUploads.length ? `（${staleUploads.length}）` : ""}
            </Button>
            <Button
              size="sm"
              variant="outline"
                          ...
                          <div className="flex justify-end gap-1">
                            ...
                            {u.status !== "READY" ? (
                              <Button
                                ...
                                disabled={!!refinalizing?.startsWith(u.id)}
                              >
                                <PlayCircle
